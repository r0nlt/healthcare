...
class RadiationEnvironment {
   public:
    ...
    {
        // C++14 compatible version of std::clamp
        solar_activity_ = std::max(0.0, std::min(activity, 1.0));
    }
    ...
    double calculateSEUProbability(const OrbitalPosition& position) const
    {
        ...
        double seu_prob = flux.electron_flux * electron_factor + flux.proton_flux * proton_factor +
