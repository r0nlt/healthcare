...
double calculateZeroPointEnergyContribution(double hbar, double mass, double lattice_constant,
                                            double temperature)
{
    // Convert lattice constant from Å to m
    double lattice_m = lattice_constant * 1e-10;

    // Calculate angular frequency using harmonic oscillator model
    // ω = sqrt(k/m), where k is spring constant estimated from lattice
    double k_estimate = 10.0;  // Approximate spring constant (N/m)
    double omega = std::sqrt(k_estimate / mass);

    // Zero-point energy: E_0 = 0.5 * hbar * omega
    double zpe = 0.5 * hbar * omega;

    // Apply thermal correction (simplified model)
    double thermal_factor = 1.0 + temperature / 300.0;

    return zpe * thermal_factor;
}
